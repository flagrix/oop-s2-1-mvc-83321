...
public class CourseEnrolmentDbTests
{
    ...
    [Fact]
    public async Task UniqueEnrolment_IsEnforced_AtAppLevel()
    {
        // InMemory ne gère pas les index uniques — on vérifie la logique applicative
        var (ctx, student, course) = await SeedAsync(nameof(UniqueEnrolment_IsEnforced_AtAppLevel));
        ctx.CourseEnrolments.Add(new CourseEnrolment { StudentProfileId = student.Id, CourseId = course.Id });
        await ctx.SaveChangesAsync();

        // Vérifier qu'un doublon existe déjà avant d'en ajouter un
        var alreadyEnrolled = await ctx.CourseEnrolments
...
public class AssignmentResultDbTests
{
    ...
    [Fact]
    public async Task UniqueAssignmentResult_IsEnforced_AtAppLevel()
    {
        var (ctx, assignment, student) = await SeedAsync(nameof(UniqueAssignmentResult_IsEnforced_AtAppLevel));
        ctx.AssignmentResults.Add(new AssignmentResult { AssignmentId = assignment.Id, StudentProfileId = student.Id, Score = 80 });
        await ctx.SaveChangesAsync();

        var alreadyExists = await ctx.AssignmentResults
            .AnyAsync(r => r.AssignmentId == assignment.Id && r.StudentProfileId == student.Id);
...
public class ExamResultDbTests
{
    ...
    [Fact]
    public async Task UniqueExamResult_IsEnforced_AtAppLevel()
    {
        var (ctx, exam, student) = await SeedAsync(nameof(UniqueExamResult_IsEnforced_AtAppLevel));
        ctx.ExamResults.Add(new ExamResult { ExamId = exam.Id, StudentProfileId = student.Id, Score = 150 });
        await ctx.SaveChangesAsync();

        var alreadyExists = await ctx.ExamResults
            .AnyAsync(r => r.ExamId == exam.Id && r.StudentProfileId == student.Id);
        Assert.True(alreadyExists);
    }
...
public class FacultyCourseAssignmentDbTests
{
    ...
    [Fact]
    public async Task UniqueFacultyCourseAssignment_IsEnforced_AtAppLevel()
    {
        var (ctx, faculty, course) = await SeedAsync(nameof(UniqueFacultyCourseAssignment_IsEnforced_AtAppLevel));
        ctx.FacultyCourseAssignments.Add(new FacultyCourseAssignment { FacultyProfileId = faculty.Id, CourseId = course.Id });
        await ctx.SaveChangesAsync();

        var alreadyExists = await ctx.FacultyCourseAssignments
